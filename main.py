...
"""
__author__ = "Michael Perkins"
__email__ = "perkim10@my.erau.edu"
__version__ = "1.0"
...
def search(file_list):
  ...
  """
  lol = []
  fS = list(map(getsize, file_list))
  file_list = list(filter(lambda x: 1 < fS.count(getsize(x)), file_list))
  while 0 < len(file_list):
      tL = [file_list.pop(0)]
      for i in range(len(file_list) - 1, -1, -1):
          if compare(tL[0], file_list[i]):
              tL.append(file_list.pop(i))
      if len(tL) > 1:
          lol.append(tL)
  return lol
...
def report(lol):
  ...
  if len(lol) > 0:
      ...
      print(f"\nThe most disk space that could be recovered is {mxSp}, by deleting copies of this file: \n{max(lol,key=lambda x : sum([getsize(i) for i in x]))[0]}")
      print(f"Here are the {len(mstSp) - 1} copies")
...
if __name__ == '__main__':
   ...
   path = "C:\\Users\\deade\\PycharmProjects\\pythonProject5\\images"
